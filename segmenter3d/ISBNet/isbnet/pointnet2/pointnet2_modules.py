...
class PointnetSAModuleVotesSeparate(nn.Module):
    ...
    def group_points(self, xyz, features, inds=None, npoint_new=None):
        temp = self.npoint
        if npoint_new is not None:
            self.npoint = npoint_new
        xyz_flipped = xyz.transpose(1, 2).contiguous()
        if inds is None:
        ...
        if not self.ret_unique_cnt:
            return new_xyz, grouped_features, grouped_xyz, inds
        else:
            return new_xyz, grouped_features, grouped_xyz, inds, unique_cnt
...
class SetAbstractionPointNext(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.grouper2 = pointnet2_utils.QueryAndGroup(
            radius * 2,
            nsample * 2,
            use_xyz=use_xyz,
            ret_grouped_xyz=True,
        ...
        if use_xyz and len(mlp_spec1) > 0:
            mlp_spec1[0] += 3

        self.mlp_module1 = pt_utils.SharedMLP(mlp_spec1, bn=bn)

        mlp_spec2 = [mlp_dim * 2, mlp_dim * 2]
        if use_xyz and len(mlp_spec2) > 0:
            mlp_spec2[0] += 3
        self.mlp_module2 = pt_utils.SharedMLP(mlp_spec2, bn=bn, activation=None)

        mlp_module3 = [
            pt_utils.Conv1d(in_size=mlp_dim * 2, out_size=mlp_dim * 2 * 4, bn=True),
            pt_utils.Conv1d(
                in_size=mlp_dim * 2 * 4,
                out_size=mlp_dim * 2,
                bn=True,
    ...
    def forward(self, new_xyz: torch.Tensor, grouped_features: torch.Tensor = None) -> torch.Tensor:
        # xyz_flipped = xyz.transpose(1, 2).contiguous()

        # # NOTE subsample
        # if inds is None:
        #     inds = pointnet2_utils.furthest_point_sample(xyz, self.npoint)
        # # new_xyz = pointnet2_utils.gather_operation(xyz.transpose(1, 2).contiguous(), inds).transpose(1, 2).contiguous()

        # # NOTE group 1st
        # new_xyz = torch.gather(xyz, 1, inds[:, :, None].expand(-1, -1, xyz.shape[-1]).long()) # (B, npoints, 3)
        # # new_features = torch.gather(features, 2, inds[:, None, :].expand(-1, features.shape[1], -1).long()) # (B, C, npoints)
        # grouped_features, grouped_xyz = self.grouper1(xyz, new_xyz, features)
        ...
        # NOTE group 2nd
        grouped_features2, grouped_xyz2 = self.grouper2(new_xyz, new_xyz, new_features)
        ...
        new_features3 = self.skip_act(new_features3 + identity)
