...
def init_foundation_models(cfg):
    ...
    '''
    # CLIP
    clip_adapter, _, clip_preprocess = open_clip.create_model_and_transforms(cfg.foundation_model.clip_model, pretrained=cfg.foundation_model.clip_checkpoint)
    clip_adapter = clip_adapter.cuda()
    # Grounding DINO
    grounding_dino_model = load_model(cfg.foundation_model.grounded_config_file, cfg.foundation_model.grounded_checkpoint, device='cuda')
    # Segment Anything
...
def gen_grounded_mask_and_feat(scene_id, clip_adapter, clip_preprocess, grounding_dino_model, sam_predictor, class_names, cfg, gen_feat=True):
    '''
    ...
    for i in trange(0, len(scannet_loader), cfg.data.img_interval):
        ...
        for cls_name in segments:
            boxes, confidences = get_grounding_output(grounding_dino_model, image_infer, '.'.join(cls_name), 
                                                      cfg.foundation_model.box_threshold, cfg.foundation_model.text_threshold, 
        ...
        l, t, r, b = boxes_filt[:, 0].clip(0), boxes_filt[:, 1].clip(0), boxes_filt[:, 2].clip(min=0, max=W), boxes_filt[:, 3].clip(min=0, max=H)
        ...
        if False:
            ...
            plt.axis('off')
            # plot out
            plt.savefig(os.path.join('../debug/' + scene_id + '/sam_'+str(i)+'.jpg'), bbox_inches="tight", dpi=300, pad_inches=0.0)
...
if __name__=='__main__':
    ...
    with torch.cuda.amp.autocast(enabled=cfg.fp16):
        for scene_id in tqdm(scene_ids):
            ...
            grounded_data_dict, grounded_features = gen_grounded_mask_and_feat(scene_id, clip_adapter, clip_preprocess, grounding_dino_model, sam_predictor, class_names=INSTANCE_CAT_SCANNET_200, cfg=cfg)
