...
def get_final_instances(cfg, text_features, cluster_dict=None, use_2d_proposals=False, use_3d_proposals=True, only_instance=True):
    '''
    ...
    with torch.cuda.amp.autocast(enabled=cfg.fp16):
        ...
        for scene_id in tqdm(scene_ids):
            ...
            if True:
                ...
                torch.save(
                    cluster_dict, 
                    os.path.join(save_dir_cluster, f"{scene_id}.pth")
                )
        ...
        if evaluate_openvocab:
            scan_eval.evaluate(res, gtsem, gtinst, exp_path=os.path.join(cfg.exp.save_dir, cfg.exp.exp_name, cfg.exp.final_output))
        if evaluate_agnostic:
            pass
