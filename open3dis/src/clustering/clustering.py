...
import pycocotools
from open3dis.src.clustering.clustering_utils import resolve_overlapping_masks, \
...
# Hierachical merge
def hierarchical_agglomerative_clustering(pcd_list, left, right, spp, n_spp, n_points, detic=False, visi=0.9, simi=0.5,):
    ...
    if left == right:
        ...
        total_spp_points = torch_scatter.scatter(
            (mapping[:,3] == 1).float(),
            spp,
            dim=0,
            reduce='sum'
        ...
        for m ,mask in enumerate(masks):
            ...
            highlight_points = idx[mask[mapping[idx][:, [1, 2]][:, 0], mapping[idx][:, [1, 2]][:, 1]].nonzero(as_tuple=True)[0]].long()

            sieve_mask = torch.zeros((n_points), device=device)
            sieve_mask[highlight_points] = 1

            num_related_points = torch_scatter.scatter(
                sieve_mask.float(),
                spp,
                dim=0,
                reduce='sum'
            ...
            spp_weights = torch.where(total_spp_points == 0, torch.tensor(0.0).cuda(), num_related_points / total_spp_points)
    ...
    graph_1_onehot, weight_1 = hierarchical_agglomerative_clustering(pcd_list, left, mid, spp, n_spp, n_points, detic=detic)
    graph_2_onehot, weight_2 = hierarchical_agglomerative_clustering(pcd_list, mid + 1, right, spp, n_spp, n_points, detic=detic)
...
# Sequential merge
def sequential_feature_overlap_feat_agg(pcd_list, left, right, spp, n_spp, n_points, detic=False, visi=0.5, simi=0.5,):
...
def process_hierarchical_agglomerative(scene_id, cfg):
    ...
    pointcloud_mapper = PointCloudToImageMapper(
        image_dim=img_dim, 
        intrinsics = scannet_loader.global_intrinsic,
    ...
    dc_feature_spp = torch_scatter.scatter(
        dc_feature,
        spp,
        dim=0,
        reduce='sum'
    ...
    for i in trange(0, len(scannet_loader), cfg.data.img_interval):
        ...
        if frame_id not in groundedsam_data_dict.keys(): continue
        ...
        if 'scannet' in cfg.data.dataset_name:
            new_mapping = scaling_mapping(torch.squeeze(mapping[:, 1:3]), img_dim[1], img_dim[0], rgb_img_dim[0], rgb_img_dim[1])
    ...
    num_instance = 0
    groups, weights = hierarchical_agglomerative_clustering(pcd_list, 0, len(pcd_list)-1, spp, n_spp, n_points, detic=False, visi=visi, simi=simi)
